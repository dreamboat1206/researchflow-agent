...
def test_search_figures_by_image_text_formats_qdrant_hits() -> None:
    agent = FigureRetrievalAgent(
        figure_store=FakeFigureStore(),
        figure_image_store=FakeFigureImageStore(),
    )

    results = agent.search_figures_by_image_text("Transformer architecture", top_k=1)
